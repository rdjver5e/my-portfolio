...
function SectionHeading({ children }: { children: string }) {
  return (
    <h2
      className="text-[11px] font-bold tracking-[0.04em] uppercase mb-2 flex items-center gap-2"
      style={{ color: BLUE, fontFamily: 'Inter, system-ui, sans-serif' }}
    >
      <span className="w-[2px] h-[11px] rounded-full shrink-0" style={{ background: '#8B5CF6' }} aria-hidden="true" />
      {children}
    </h2>
  )
}
...
function BulletList({ items, color }: { items: string[]; color: string }) {
  return (
    <ul className="space-y-1.5">
      {items.map((t, i) => (
        <li key={i} className="flex gap-2 text-[10.5px] leading-[1.65]" style={{ color }}>
...
export default function ResumePage({ darkMode = true }: { darkMode?: boolean }) {
  ...
  return (
    <div className="min-h-screen transition-colors duration-300" style={{ background: PAGE_BG }}>
      ...
      <main className="max-w-[880px] mx-auto px-4 md:px-6 pt-28 pb-16">
        <div
          className="resume-sheet overflow-hidden transition-colors duration-300"
          style={{ border: `1px solid ${BORDER}`, borderRadius: RADIUS }}
        >
          <div className="px-7 md:px-10 py-8 md:py-10">
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
              ...
              <div
                className="text-[10px] leading-[1.7] md:text-right shrink-0"
                style={{ color: BODY, fontFamily: 'Inter, system-ui, sans-serif' }}
              >
            ...
            <div className="mt-6 h-px w-full" style={{ background: BORDER }} />

            {/* Two columns */}
            <div className="mt-8 grid md:grid-cols-[1.7fr_0.95fr] gap-8 md:gap-10">
              {/* Left */}
              <div>
                ...
                <div className="space-y-5 mb-1">
                  {((resume as any).experiences ?? [(resume as any).experience]).map((exp: any, idx: number) => (
                    <div key={idx}>
                      <p className="text-[11px] font-bold leading-tight" style={{ color: BODY }}>
                        {exp.org}
                      </p>
                ...
                <div className="mt-5">
                  ...
                  <div className="space-y-4">
                    {resume.projects.map((p) => (
                      <div key={p.name}>
                        <p className="text-[11px] font-bold" style={{ color: BODY }}>
              ...
              <div>
                ...
                <ul className="space-y-1 mb-6">
                  {resume.skills.map((s) => (
                    <li key={s} className="flex gap-2 text-[10.5px] leading-[1.6]" style={{ color: BODY }}>
                ...
                <ul className="space-y-2.5 mb-6">
                  {resume.education.map((e, i) => (
                    <li key={i} className="flex gap-2 text-[10.5px] leading-[1.55]" style={{ color: BODY }}>
                ...
                <ul className="space-y-1">
                  {resume.languages.map((l) => (
                    <li key={l} className="flex gap-2 text-[10.5px] leading-[1.6]" style={{ color: BODY }}>
                      <span className="shrink-0" aria-hidden="true">
                        •
                      </span>
                      <span>{l}</span>
